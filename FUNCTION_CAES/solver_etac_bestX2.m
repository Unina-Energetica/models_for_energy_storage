function [pass,NC_c1] = solver_etac_bestX2(mappa_c1,mappa_c2,f_Eta_c1,f_Nc_c2,f_Eta_c2,beta_1,beta_2,Nsol,P_disp,CINC,T_oil,T_water,m_oil,m_water)
...
for y = 1:length(N_aly)
    ...
    [mc_c1test(y),mc_c2test(y),eta_b1(y),eta_b2(y),~,V_shaft(:,y),~,NC_c2] = solver_compressX2_explicito_ts(f_Eta_c1,f_Nc_c2,f_Eta_c2,beta_1,beta_2,N_aly(y),mappa_c1,mappa_c2,CINC,T_oil,T_water,m_oil,m_water);
    ...
    if flag2 ~= -2  &&  V_shaft(1,y) > 0  &&  V_shaft(2,y) > 0  &&  P_shaft(y) <= P_disp
        ...
        eta_best(y) = (V_shaft(1,y)*eta_b1(y) + V_shaft(2,y)*eta_b2(y))/P_shaft(y);
