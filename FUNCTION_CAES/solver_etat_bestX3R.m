function [pass,Nc_T1] = solver_etat_bestX3R(mappa_t1,mappa_t2,mappa_t3,f_Eta_c_t1,f_Nc_t2,f_Eta_c_t2,f_Nc_t3,f_Eta_c_t3,beta_1,beta_2,beta_3,Nsol,P_ob,TINC,Ts_in,To_in,m_heater_sun2,m_heater_sun3)
...
for y = 1:length(N_aly)
    ...
    [mc_t1test(y),mc_t2test(y),mc_t3test(y),eta_b1(y),eta_b2(y),eta_b3(y),V_shaft(:,y),~,Nc_T2,Nc_T3,~,~,~,~] = solver_TrenoX3_explicitoR(f_Eta_c_t1,f_Nc_t2,f_Eta_c_t2,f_Nc_t3,f_Eta_c_t3,beta_1,beta_2,beta_3,N_aly(y),mappa_t1,mappa_t2,mappa_t3,TINC,Ts_in,To_in,m_heater_sun2,m_heater_sun3);
    ...
    if flag2 ~= -2  &&  flag3 ~= -2  &&  V_shaft(1,y) > 0  &&  V_shaft(2,y) > 0  &&  V_shaft(3,y) > 0  &&  P_shaft(y) <= P_ob
...
if graf_b == 1
    ...
    plot(mc_t1test(1:end),beta_test1(1:end),'xk','LineWidth',2)
    plot(mc_t1test(ib),beta_test1(ib),'xr','LineWidth',2)
    hold off
    ...
    plot(mc_t2test(1:end),beta_test2(1:end),'xk','LineWidth',2)
    plot(mc_t2test(ib),beta_test2(ib),'xr','LineWidth',2)
    hold off
    ...
    plot(mc_t3test(1:end),beta_test3(1:end),'xk','LineWidth',2)
    plot(mc_t3test(ib),beta_test3(ib),'xr','LineWidth',2)
    hold off               
    % pause(1)
