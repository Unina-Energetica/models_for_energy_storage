function dyadta = Absorption_for(ta, ya, m_H2_in, Twa, DTml, m_s, A, U, Pa, Ca, Ea, R, M_H2, SC, M_MH, Cp_H2, Cp_s, Tin_a, DHa, DSa, sl, P0)
...
if m_H2_in == 0
    d_m_H2_a_dta = 0;                                     % Hydrogen mass        [kg]
    d_m_MH_a_dta = 0;                                     % Metal hydride mass   [kg]
    d_T_a_dta    = (-(A*U*DTml))/(m_H2_a*Cp_H2+m_s*Cp_s); % System's temperature [K]  
else
    d_m_H2_a_dta = m_H2_in-r_a*m_s*((M_H2*SC)/M_MH);                                                     % Hydrogen mass        [kg]
    d_m_MH_a_dta = r_a*m_s;                                                                              % Metal hydride mass   [kg]
    d_T_a_dta    = ((m_H2_in)*Cp_H2*(Tin_a-T_a)-(A*U*DTml)-DHa*r_a*m_s*SC/M_MH)/(m_H2_a*Cp_H2+m_s*Cp_s); % System's temperature [K]  
end
