function dyddtd = Desorption_for(td, yd, m_H2_out, Twd, DTml, Cd, Ed, R, Pd, m_s, M_H2, SC, M_MH, Cp_H2, Cp_s, A, U, DHd, DSd, sl, P0)
...
% ODE
if m_H2_out == 0 
    d_m_H2_d_dtd = 0;                                % Hydrogen mass        [kg]
    d_m_MH_d_dtd = 0;                                % Metal hydride mass   [kg]
    d_T_d_dtd = (A*U*DTml)/(m_H2_d*Cp_H2+m_s*Cp_s);  % System's temperature [K] 
else
    d_m_H2_d_dtd = -m_H2_out-r_d*m_s*((M_H2*SC)/M_MH);                  % Hydrogen mass        [kg]
    d_m_MH_d_dtd = r_d*m_s;                                             % Metal hydride mass   [kg]
    d_T_d_dtd = (A*U*DTml+DHd*r_d*m_s*SC/M_MH)/(m_H2_d*Cp_H2+m_s*Cp_s); % System's temperature [K]
end
