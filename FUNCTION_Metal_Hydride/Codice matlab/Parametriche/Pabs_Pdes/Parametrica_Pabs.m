clear all; clc;
% Parametrizzazione rispetto a m_w
...
grid on;
xlabel('Time [s]', 'FontName','Times New Roman','FontWeight','bold', 'FontSize',13);
ylabel('m_{MH} [K]', 'FontName','Times New Roman','FontWeight','bold', 'FontSize',13);
title('Metal hydride mass at Varying m_w', 'FontName','Times New Roman','FontWeight','bold', 'FontSize',13);
legendInfo = {};
...
for j = 1:numel(Pa)
    ...
    for i = 1:numel(t)-1
        ...
        % Function handle
        ode_fun_a = @(ta, ya) Assorbimento_for(ta, ya, m_H2_in, Twa(1), DTml(i), Pa(j));
        ...
        Tw_out(i+1) = Twa(1) + (DTml(i+1) * U * A) / (m_w * Cp_w * 1000);
    end

    %subplot(3, 1, 1);
    figure(1)
    plot(t, T_a);
    legendInfo{j} = sprintf('Pa = %.1f Pa', Pa(j));
